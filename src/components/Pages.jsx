...
export default function Pages({page, handlePageChange, totalPages, start}){
    ...
    return(
        <div className="d-flex w-100 justify-content-center align-items-center py-3 flex-wrap">
            ...
            <span className={`${page > totalPages - 4 ? 'd-none' : ''}`}>...</span><button className={`btn px-3 mx-3 ${page > totalPages - 3 ? 'd-none' : 'btn-light2'}`} onClick={() => changePage(totalPages)}>{totalPages}</button>
